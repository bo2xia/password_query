...
class LoginForm(FlaskForm):
    username = StringField('账号', validators=[DataRequired(message='请输入账号'), Length(min=3, max=20)])
...
class BatchGenerateForm(FlaskForm):
    username = SelectField('账号', validators=[DataRequired(message='请选择账号')], choices=[])
...
class AccountForm(FlaskForm):
    username = SelectField('账号', validators=[DataRequired(message='请选择账号')], choices=[], render_kw={'class': 'form-control'})
    new_username = StringField('新账号', validators=[Length(min=3, max=20, message='账号长度必须为3-20个字符')])
    password = PasswordField('密码', validators=[Length(min=0, max=128, message='密码长度不能超过128个字符')])
    action = SelectField('操作', choices=[('add', '添加账号'), ('modify_random', '修改密码为随机')], validators=[DataRequired(message='请选择操作')])
    submit = SubmitField('提交')

    def validate(self, extra_validators=None):
        if not super().validate(extra_validators):
            return False
        if self.action.data == 'add' and not self.new_username.data:
            self.new_username.errors.append('请输入新账号')
            return False
        if self.action.data == 'add' and not self.password.data:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not is_valid_username(username):
            logging.warning(f"账号格式无效: {username}")
            return render_template('login.html', form=form, error='账号格式无效')
...
def accounts():
    if redirect_response := ensure_admin_session():
        return redirect_response
    form = AccountForm()
    form.username.choices = [('', '请选择账号')] + [(account.username, account.username) for account in Account.query.all()]
    error = success = None
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            logging.info(f"接收到表单提交: action={action}, username={username}, new_username={new_username}")
            
            if action == 'add':
                if not is_valid_username(new_username):
                    error = '新账号格式无效'
                    logging.warning(f"账号格式无效: {new_username}")
                elif Account.query.get(new_username):
                    error = '账号已存在'
                    logging.warning(f"添加账号失败: 账号已存在 {new_username}")
                else:
                    try:
                        account = Account(
                        ...
                        db.session.commit()
                        success = f'账号 {new_username} 添加成功'
                        logging.info(f"账号添加成功: {new_username}")
                        # 更新 SelectField 选项
                        form.username.choices = [('', '请选择账号')] + [(account.username, account.username) for account in Account.query.all()]
                    except Exception as e:
                        error = f'数据库错误: {str(e)}'
                ...
                if not username:
                    ...
                    if account:
                        ...
                        error = '账号不存在'
                        logging.warning(f"修改密码失败: 账号不存在 {username}")
